...
func (db *DB) CreateApp(ctx context.Context, clusterID, name, namespace, image string, replicas int, port *int, envVars []byte) (*App, error) {
	var a App
	err := db.GetContext(ctx, &a, `
		INSERT INTO apps (cluster_id, name, namespace, image, replicas, port, env_vars, status)
		VALUES ($1, $2, $3, $4, $5, $6, $7, 'pending')
		RETURNING *
	`, clusterID, name, namespace, image, replicas, port, envVars)
	return &a, err
}

func (db *DB) UpdateApp(ctx context.Context, id, image string, replicas int, envVars []byte) (*App, error) {
	var a App
	err := db.GetContext(ctx, &a, `
		UPDATE apps SET image = $1, replicas = $2, env_vars = $3, updated_at = NOW()
		WHERE id = $4 RETURNING *
	`, image, replicas, envVars, id)
	return &a, err
}
